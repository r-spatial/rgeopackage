# From https://www.geopackage.org/spec131/index.html
#
# License Agreement
#
...
# THIRD PARTY RIGHTS. THE COPYRIGHT HOLDER OR HOLDERS INCLUDED IN THIS NOTICE DO
# NOT WARRANT THAT THE FUNCTIONS CONTAINED IN THE INTELLECTUAL PROPERTY WILL
# MEET YOUR REQUIREMENTS OR THAT THE OPERATION OF THE INTELLECTUAL PROPERTY WILL
# BE UNINTERRUPTED OR ERROR FREE. ANY USE OF THE INTELLECTUAL PROPERTY SHALL BE
...
# enforceable, and as so modified the entire Agreement shall remain in full
# force and effect. No decision, action or inaction by LICENSOR shall be
# construed to be a waiver of any rights or remedies available to it. None of
# the Intellectual Property or underlying information or technology may be
# downloaded or otherwise exported or reexported in violation of U.S. export
...
# or use the Intellectual Property, and you represent that you have complied
...
spec131_html <-
  htmldf::html_df("https://www.geopackage.org/spec131/index.html")

spec131_tables <- map(
  spec131_html$tables[[1]],
  ~ janitor::clean_names(.x, "snake")
)

spec131_tables_df <-
  bind_cols(
    "id" = seq_along(spec131_tables),
    "name" = names(spec131_tables)
  )

spec131_tables_df$table <- spec131_tables

# link table for reference
spec131_links <-
  dplyr::filter(
    spec131_html$links[[1]],
    stringr::str_detect(href, "^https://www.geopackage.org/spec131")
  ) |>
  dplyr::distinct(.keep_all = TRUE)

gpkg_metadata_spec <- list(
  "metadata_scopes" = spec131_tables_df[["table"]][[137]]
)

spec131 <-
  list(
    "gpkg_metadata" = gpkg_metadata_spec
  )

usethis::use_data(spec131, internal = TRUE, overwrite = TRUE)
